...
Run **`holocron ci`** — it runs exactly the merge-gating checks locally, in CI order, and exits non-zero on the first failure. Finding failures after pushing wastes a review round trip. (Equivalent: `pnpm typecheck && pnpm lint && pnpm test && pnpm build`.) `protection: "strict"` repos install a `pre-push` git hook that runs it automatically; `git push --no-verify` bypasses that hook once.
